...
//add new product
router.post("/",verifyToken,verifyTokenAndAdmin ,uploadOptions.single('image') , async(req,res)=>{
    ...
    // product code unique one eg p01 p02 mongodb
    const lastProduct = await Product.findOne().sort({_id:-1});
    const lastProductCode = lastProduct.productCode;
    const lastProductCodeNumber = parseInt(lastProductCode.substring(1));
    const newProductCodeNumber = lastProductCodeNumber + 1;
    const newProductCode = "p" + newProductCodeNumber;
...
//update product
router.put("/:id", verifyToken,verifyTokenAndAdmin ,uploadOptions.single('image') ,async (req,res)=>{
    //id validation
...
router.get("/", async(req,res)=>{
    ...
    const productList = await Product.find(filter).populate('category').sort({createdAt: -1});
...
router.delete("/:id",verifyToken,verifyTokenAndAdmin , async (req,res)=>{
    try{
        ...
        if(!deletedProduct){
            ...
            // get the path of the image to be deleted
            const oldImagePath = path.join(__dirname, `../public/${deletedProduct.image}`);
            // delete the image
            fs.unlink(oldImagePath, (err) => {
                if (err) { 
                    console.error(err)
